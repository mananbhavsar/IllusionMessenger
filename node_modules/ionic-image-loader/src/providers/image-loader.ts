...
export class ImageLoader {
  ...
  clearCache(): void {
    ...
    const clear = () => {
      ...
      this.file.removeRecursively(this.dataDirectory, this.config.cacheDirectoryName)
        .then(() => {
  ...
  private processQueue() {
    ...
    const localPath = this.dataDirectory + this.config.cacheDirectoryName + '/' + this.createFileName(currentItem.imageUrl);
  ...
  private indexCache(): Promise<void> {
    ...
    return this.file.listDir(this.dataDirectory, this.config.cacheDirectoryName)
  ...
  private removeFile(file: string): Promise<any> {
    return this.file
      .removeFile(this.dataDirectory + this.config.cacheDirectoryName, file)
      .then(() => {
        if (this.isWKWebView && !this.isIonicWKWebView) {
          return this.file
  ...
  private getCachedImagePath(url: string): Promise<string> {
    return new Promise<string>((resolve, reject) => {
      ...
      // get full path
      const dirPath = this.dataDirectory + this.config.cacheDirectoryName,
  ...
  private createCacheDirectory(replace: boolean = false): Promise<any> {
    ...
    if (replace) {
      ...
      cacheDirectoryPromise = this.file.createDir(this.dataDirectory, this.config.cacheDirectoryName, replace);
    } else {
      // check if the cache directory exists.
      // if it does not exist create it!
      cacheDirectoryPromise = this.cacheDirectoryExists(this.dataDirectory)
        .catch(() => this.file.createDir(this.dataDirectory, this.config.cacheDirectoryName, false));
    }
